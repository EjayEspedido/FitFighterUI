// src/gameplay/play/PlayCombo.tsx
...
export default function PlayCombo() {
  ...
  useEffect(() => {
    if (isEndless) {
      console.debug("[PlayCombo] endless mode - no countdown");
      return undefined;
    }
    if (durationLeftSec === null) {
      console.debug(
        "[PlayCombo] no timer configured (durationLeftSec is null)"
      );
      return undefined;
    }
    ...
    console.debug("[PlayCombo] starting countdown", { durationLeftSec });
    ...
    return () => {
      ...
      console.debug("[PlayCombo] cleared countdown (cleanup)");
    };
  }, [isEndless, paused, durationLeftSec]);

  // watch lives
  useEffect(() => {
    console.debug("[PlayCombo] lives changed", { livesLeft });
    if (!ongoing) return;
    if (livesLeft <= 0) {
      endGame("lives");
    }
  ...
  // watch duration
  useEffect(() => {
    console.debug("[PlayCombo] durationLeftSec changed", { durationLeftSec });
    if (!ongoing) return;
  ...
  function endGame(reason?: string) {
    if (endedRef.current) {
      console.debug("[PlayCombo] endGame called but already ended", { reason });
      return;
    }
    ...
    console.debug("[PlayCombo] ending game", { reason, result });

    // short delay to let UI update then navigate
    setTimeout(() => {
      navigate("/play/end/combo", { state: { result } });
    }, 120);
  }
  ...
  return (
    <div className="page start-page" style={{ padding: 18 }}>
      <h1 className="title">Combo Mode — Play</h1>
